#!/usr/bin/env python3
"""
Script to create combinations of RGB with different LiDAR modalities
"""
...
def main():
    print("Creating combination datasets...")
    ...
    combinations = {
        ...
        6: 'nearir'     # Combination 6: RGB + Near-IR (alternative)
    }
    
    # Create each combination
    for combo_num, modality in combinations.items():
        create_combination_dataset(combo_num, modality, source_dir, ".")
    
    print("All combinations created successfully!")
...
def process_split(rgb_path, modality_path, labels_path, output_path, split, combination_num, modality_name):
    """Process a specific split (train/valid/test)"""
...
Image format: Combined RGB and {modality_name} horizontally concatenated
Label format: YOLO format (.txt files)
Target class: Pole

Modalities:
- RGB: Combined color images (Near-IR + Signal + Reflectivity as RGB)
- {modality_name}: {"Near-infrared" if modality_name == "nearir" else 
                   "Range/distance data" if modality_name == "range" else
                   "Reflectivity data" if modality_name == "reflec" else
                   "Signal intensity" if modality_name == "signal" else modality_name}
"""
